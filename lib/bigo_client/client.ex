defmodule BigoClient.Client do
  ...
  def main do
    Node.set_cookie(System.get_env("BG_COOKIE"))
    :net_kernel.start([mnode(hname), :longnames])
    start
  end
  ...
  defp call(m, arg \\ []) do
    :rpc.call(mnode, BigoServer.Server, m, arg)
  end
  ...
  defp log(s) do
    GenServer.cast({BL, mnode}, {:log, "#{hname} -> #{s}"})
  end
